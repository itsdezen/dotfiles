...
check_font() {
  [[ -f "$HOME/Library/Fonts/FantasqueSansMNerdFont-Regular.ttf" ]] && return 0 || return 1
}
...
EOF
  ...
  OPTIONS[1]="Fantasque Sans Mono Nerd Font"
  ...
  OPTIONS[7]="Terminal.app Profile (Clear Dark with Fantasque Sans Mono)"
...
if [[ $INSTALL_FONT -eq 1 ]] && [[ "$SKIP_BREW" == false ]]; then
  header "Fantasque Sans Mono Nerd Font"
...
if [[ $INSTALL_TERMINAL -eq 1 ]]; then
  ...
  echo "     ${GREEN}✓${RESET} Font: Fantasque Sans Mono Nerd Font (14pt)"
  ...
  echo "     ${CYAN}Fantasque Sans Mono Nerd Font${RESET}"
